    ...
    try {
        switch (name) {
            ...
            case "scrape_real_estate_listings": {
                const { location, propertyType, priceRange, beds, baths, limit = 50 } = args;
                try {
                    ...
                    if (priceRange?.min || priceRange?.max) {
                        const priceFilter = [];
                        if (priceRange.min)
                            priceFilter.push(priceRange.min);
                        if (priceRange.max)
                            priceFilter.push(priceRange.max);
                        searchParams.push(priceFilter.join("-"));
                    }
                    if (beds)
                        searchParams.push(`${beds}-beds`);
                    if (baths)
                        searchParams.push(`${baths}-baths`);
            ...
            case "scrape_market_data": {
                const { location, dataType = "prices", timeRange = "6-months" } = args;
                try {
            ...
            case "scrape_agent_data": {
                const { location, specialty, limit = 25 } = args;
                try {
