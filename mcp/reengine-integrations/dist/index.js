...
class WhatsAppIntegration {
    ...
    async sendMessage(to, message, type = "text", mediaUrl) {
        try {
            const payload = {
...
class LinkedInIntegration {
    ...
    async getProfile(profileId) {
        try {
            const response = await axios.get(`${this.baseUrl}/people/${profileId}`, {
                headers: {
                    Authorization: `Bearer ${this.accessToken}`,
                },
                params: {
    ...
    async sendConnection(profileId, message) {
        try {
            const response = await axios.post(`${this.baseUrl}/relationshipInvitations`, {
                invitee: {
                    profile: profileId,
                },
                message: message,
    ...
    async searchPeople(searchQuery) {
        try {
...
class FacebookIntegration {
    ...
    async sendMessage(pageId, userId, message) {
        try {
    ...
    async postToPage(pageId, postContent) {
        try {
...
class TelegramIntegration {
    ...
    async sendDocument(chatId, documentUrl) {
        try {
    ...
    async createChannel(channelName) {
        try {
            const response = await axios.post(`${this.baseUrl}/createChat`, {
                title: channelName,
                type: "channel",
            });
            return {
    ...
    try {
        switch (name) {
            ...
            case "whatsapp_send_message": {
                ...
                const result = await whatsapp.sendMessage(parsed.to, parsed.message, parsed.type, parsed.mediaUrl);
                return {
                    content: [
                        {
                            type: "text",
