...
const selectOptimalModel = (useCase: string, requirements?: any): string => {
  ...
  const useCaseMapping = REAL_ESTATE_USE_CASES[useCase as keyof typeof REAL_ESTATE_USE_CASES];
  if (useCaseMapping) {
    const availablePrimary = useCaseMapping.primary.filter(model => 
    ...
    const availableFallback = useCaseMapping.fallback.filter(model => 
      Object.keys(LLAMA_MODELS).includes(model)
    );
  ...
  const availableModels = Object.keys(LLAMA_MODELS).filter(modelId => 
...
async function analyzeDocument(params: any) {
  try {
