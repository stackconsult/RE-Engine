...
describe('Response Builder Utilities', () => {
  ...
  describe('Error Handler Middleware', () => {
    it('should handle errors and return proper response', () => {
      ...
      const result = errorHandlerMiddleware(error, null, mockRes as any, null);
      
      // The middleware should return the response
      assert.ok((result as { statusCode: number }).statusCode);
      assert.equal((result as { statusCode: number }).statusCode, 400);
      assert.equal((result as { data: ApiResponse<never> }).data.success, false);
      assert.equal((result as { data: ApiResponse<never> }).data.error?.code, 'VALIDATION_ERROR');
    });
    ...
    it('should handle string errors', () => {
      ...
      assert.equal((result as { data: ApiResponse<never> }).data.error?.code, 'UNKNOWN_ERROR');
      assert.equal((result as { data: ApiResponse<never> }).data.error?.message, 'String error message');
    });
  });

  describe('Type Guards and Interfaces', () => {
    it('should maintain proper response types', () => {
      const successResponse: ApiResponse<string> = ResponseBuilder.success('test');
      const errorResponse: ApiResponse<never> = ResponseBuilder.error('test');
      const paginatedResponse: PaginatedResponse<{ id: number }> = 
        ResponseBuilder.paginated([{ id: 1 }], 1, 10, 1);
      const validationResponse: ValidationErrorResponse = 
        ResponseBuilder.validationError([{ field: 'test', message: 'error' }]);
