...
describe('RE Engine Core Tests', () => {
    ...
    test('approval workflow', async () => {
        ...
        const insertResult = await db.query('INSERT INTO approvals (approval_id, status, lead_id, channel, draft_to, draft_subject, draft_content, draft_from, approved_by, approved_at, created_at, updated_at, rejection_reason) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', Object.values(approvalData));
    ...
    test('lead management', async () => {
        ...
        const insertResult = await db.query('INSERT INTO leads (lead_id, first_name, last_name, email, phone_e164, city, province, source, tags, status, created_at, updated_at, metadata) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', Object.values(leadData));
