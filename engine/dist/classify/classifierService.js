...
export class ClassifierService {
    ...
    calculateScore(keywords, targetKeywords) {
        if (keywords.length === 0)
            return 0;
        const matches = keywords.filter(keyword => targetKeywords.some(target => target.includes(keyword) || keyword.includes(target)));
