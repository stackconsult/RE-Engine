...
export class CSVAdapter {
     ...
     */
    async write(filename, records, headers) {
        try {
            const filePath = join(this.options.dataDir, filename);
     ...
     */
    async update(filename, updates, keyField) {
        try {
            const existing = await this.read(filename);
            if (!existing.success) {
                return { success: false, recordsWritten: 0, error: existing.error };
            ...
            const updateMap = new Map(updates.map(record => [String(record[keyField]), record]));
            ...
            const existingKeys = new Set(existing.records.map(record => String(record[keyField])));
            const newRecords = updates.filter(record => !existingKeys.has(String(record[keyField])));
     ...
     */
    async delete(filename, keys, keyField) {
        try {
            const existing = await this.read(filename);
            if (!existing.success) {
                return { success: false, recordsWritten: 0, error: existing.error };
            ...
            const filteredRecords = existing.records.filter(record => !keySet.has(String(record[keyField])));
     ...
     */
    validateStructure(filename, expectedHeaders) {
        try {
            const filePath = join(this.options.dataDir, filename);
            if (!existsSync(filePath)) {
