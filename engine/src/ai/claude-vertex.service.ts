...
export interface ClaudeTokenCountResponse {
  inputTokens: number;
}

export class ClaudeVertexService {
  private config: ClaudeVertexConfig;
  private accessToken: string | null = null;
  private tokenExpiry: number = 0;
  private anthropicClient: any = null;

  constructor(config: ClaudeVertexConfig) {
    this.config = config;
  ...
  async initialize(): Promise<void> {
    try {
      ...
      logError(error as Error, 'claude-vertex-init-failed');
      throw error;
    }
  }
  ...
  private async authenticate(): Promise<void> {
    try {
      ...
      logError(error as Error, 'claude-vertex-oauth-auth-failed');
  ...
  private async initializeAnthropicClient(): Promise<void> {
    try {
      ...
      logError(error as Error, 'claude-vertex-client-init-failed');
  ...
  async generateCompletion(request: ClaudeCompletionRequest): Promise<ClaudeCompletionResponse> {
    ...
    try {
      ...
      let response: ClaudeCompletionResponse;
      ...
      logError(error as Error, 'claude-vertex-completion-failed', {
  ...
  private async generateCompletionWithSDK(request: ClaudeCompletionRequest, model: string): Promise<ClaudeCompletionResponse> {
  ...
  private async generateCompletionManual(request: ClaudeCompletionRequest, model: string): Promise<ClaudeCompletionResponse> {
  ...
  async countTokens(request: ClaudeTokenCountRequest): Promise<ClaudeTokenCountResponse> {
    await this.ensureAuthenticated();
    ...
    try {
      ...
      logError(error as Error, 'claude-vertex-token-count-failed', {
        model: request.model || this.config.modelId
      });
      throw error;
    }
  ...
  async healthCheck(): Promise<boolean> {
    try {
      ...
      logError(error as Error, 'claude-vertex-health-check-failed');
      return false;
    }
  }
