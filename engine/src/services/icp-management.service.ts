...
import { ICPProfile } from './icp-discovery.service';
import { CSVAdapter } from '../a2d/adapters/csv-adapter';

export interface ICPCreateRequest {
  name: string;
  description: string;
  criteria: ICPProfile['criteria'];
  settings: ICPProfile['settings'];
}

export interface ICPUpdateRequest {
  name?: string;
  description?: string;
  criteria?: Partial<ICPProfile['criteria']>;
  settings?: Partial<ICPProfile['settings']>;
}

export interface ICPManagementResult {
  success: boolean;
  icp?: ICPProfile;
  error?: string;
}
...
export class ICPManagementService {
   ...
   */
  async createICP(request: ICPCreateRequest): Promise<ICPManagementResult> {
    try {
   ...
   */
  async updateICP(icpId: string, request: ICPUpdateRequest): Promise<ICPManagementResult> {
    try {
   ...
   */
  async getICP(icpId: string): Promise<ICPManagementResult> {
    try {
      ...
      const icp = result.records.find((record: any) => record.id === icpId);
      if (!icp) {
        return { success: false, error: 'ICP not found' };
      }
   ...
   */
  async listICPs(): Promise<ICPManagementResult> {
    try {
      const result = await this.csv.read('icp_profiles.csv');
      if (!result.success) {
        return { success: false, error: 'Failed to read ICP profiles' };
      ...
      const icps = result.records.map((record: any) => this.parseICPRecord(record));
   ...
   * Delete ICP profile
   */
  async deleteICP(icpId: string): Promise<ICPManagementResult> {
    try {
      const result = await this.csv.read('icp_profiles.csv');
      if (!result.success) {
        return { success: false, error: 'Failed to read ICP profiles' };
      ...
      const filteredRecords = result.records.filter((record: any) => record.id !== icpId);
   ...
   * Get ICP templates
   */
  async getICPTemplates(): Promise<ICPProfile[]> {
    return [
      {
   ...
   */
  async cloneFromTemplate(templateId: string, name: string, description: string): Promise<ICPManagementResult> {
    try {
   ...
   */
  validateICP(icp: ICPProfile): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
   ...
   * Save ICP to CSV
   */
  private async saveICP(icp: ICPProfile): Promise<void> {
    ...
    const records = existing.records.filter((record: any) => record.id !== icp.id);
   ...
   */
  private icpToRecord(icp: ICPProfile): any {
    return {
      id: icp.id,
      name: icp.name,
   ...
   */
  private parseICPRecord(record: any): ICPProfile {
    return {
      id: record.id,
      name: record.name,
