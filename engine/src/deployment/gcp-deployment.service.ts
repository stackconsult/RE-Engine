...
export class GCPDeploymentService {
  private config: GCPDeploymentConfig;
  private initialized = false;

  constructor(config: GCPDeploymentConfig) {
    this.config = config;
  }
    ...
    try {
      ...
      if (this.config.serviceAccountKey) {
        await this.executeCommand(
          `gcloud auth activate-service-account --key-file=${this.config.serviceAccountKey}`,
      ...
      logError(error as Error, 'gcp-deployment-init-failed');
      throw error;
    }
  }

  async deployToCloudRun(imageName: string, serviceName: string): Promise<DeploymentResult> {
    if (!this.initialized) {
    ...
    try {
      ...
      // Get service URL
      const { stdout: serviceInfo } = await this.executeCommand(
        `gcloud run services describe ${serviceName} --region=${this.config.region} --format="value(status.url)"`,
        'Getting service URL'
      );
      ...
      logError(error as Error, 'cloudrun-deploy-failed', { serviceName, imageName });
      return {
        success: false,
  ...
  async deployToGKE(imageName: string, deploymentName: string, namespace = 'default'): Promise<DeploymentResult> {
    if (!this.initialized) {
      await this.initialize();
    }
    ...
    try {
      ...
      if (this.config.clusterName) {
        await this.executeCommand(
          `gcloud container clusters get-credentials ${this.config.clusterName} --region=${this.config.region}`,
      ...
      const { stdout } = await this.executeCommand(
        `echo '${deploymentManifest}' | ${kubectlCommand}`,
      ...
      const { stdout: serviceInfo } = await this.executeCommand(
        `kubectl get service ${deploymentName}-service --namespace=${namespace} -o json`,
      ...
      logError(error as Error, 'gke-deploy-failed', { deploymentName, imageName });
      return {
        success: false,
  ...
  async buildAndPushImage(imageName: string, dockerfilePath = 'Dockerfile'): Promise<DeploymentResult> {
    if (!this.initialized) {
      await this.initialize();
    }
    ...
    try {
      ...
      logError(error as Error, 'image-build-failed', { imageName });
      return {
        success: false,
  ...
  async createCloudSQLInstance(instanceName: string, databaseVersion = 'POSTGRES_15'): Promise<DeploymentResult> {
    if (!this.initialized) {
      await this.initialize();
    }
    ...
    try {
      ...
      const { stdout: connectionInfo } = await this.executeCommand(
        `gcloud sql instances describe ${instanceName} --format="value(databaseVersion,region,ipAddresses.ipAddress)"`,
      ...
      logError(error as Error, 'cloudsql-create-failed', { instanceName });
      return {
        success: false,
  ...
  private async executeCommand(command: string, description: string): Promise<{ stdout: string; stderr: string }> {
  ...
  private createK8sDeploymentManifest(imageName: string, deploymentName: string, namespace: string): string {
    return `
apiVersion: apps/v1
kind: Deployment
metadata:
